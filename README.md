...
`/api/ai-assistant` is the Ask the book endpoint. It uses the same `SUPABASE_URL`, `SUPABASE_ANON_KEY`, and `ALLOWED_ORIGINS` values plus server-only `OPENROUTER_API_KEY` (optional `AI_MODEL` overrides the primary only; default MiniMax M3 Free `minimax/minimax-m3:free`, falling back to OpenRouter Free Tool Call `openrouter/free` on unavailable/rate-limit). Native builds call the deployed function through `VITE_AI_ASSISTANT_URL` — do not put `OPENROUTER_API_KEY` in any `VITE_*` variable. Apply the AI assistant quota migration before use; quota RPC failure returns 503. Questions are answered from an allowlisted, read-only catalog covering all active farm records and seasons; every query uses the caller's JWT and authoritative farm scope, and no SQL or mutation tool is exposed. OpenRouter requests deny providers that collect user data and require support for every requested parameter; prompt logging must remain disabled in the OpenRouter account. Local Ask-the-book audit rows are retained for 30 days. Free-model availability and account-wide limits are controlled by OpenRouter.
