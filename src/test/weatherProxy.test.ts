...
describe('weather proxy', () => {
  ...
  it('accepts the timed endpoint generated by historical recovery', async () => {
    const { state } = await invoke({ query: { location: '40,-90', endpoint: '2026-09-03T14:30:00' } });
    expect(state.status).toBe(200);
    expect(vi.mocked(fetch).mock.calls[0][0]).toContain('/2026-09-03T14%3A30%3A00?');
  });
  it.each(['2026-02-30T14:30:00', '2026-09-03T25:30:00', 'today/../../secret', '2026-09-03T12:60:00'])('rejects malformed endpoint %s', async endpoint => {
    const { state } = await invoke({ query: { location: '40,-90', endpoint } });
